...
export default function DetalheGerente() {
  ...
  const loadData = async () => {
    try {
      ...
      let url = `/api/dono-banca/gerentes/${gerenteId}`;
  ...
  const loadTop5Consultants = async () => {
    ...
    try {
      ...
      let url = `/api/dono-banca/gerentes/${gerenteId}/top5-consultants`;
      const params = new URLSearchParams();
      if (range?.startDate) params.append('date_from', range.startDate);
      if (range?.endDate) params.append('date_to', range.endDate);
      if (params.toString()) {
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {consultorMetrics.map(consultor => (
                  <div key={consultor.id} className="p-4 rounded-xl border border-gray-100 dark:border-gray-600 hover:border-emerald-200 dark:hover:border-emerald-700 transition-all bg-gray-50/30 dark:bg-[#333]/30">
                    ...
                    <button
                      onClick={() => router.push(`/dono-banca/consultores/${consultor.id}`)}
                    ...
                    <button
                      ...
                      className="w-full mt-2 py-2 bg-white dark:bg-[#2a2a2a] border border-gray-100 dark:border-gray-600 rounded-lg text-xs font-bold text-gray-500 dark:text-gray-400 hover:bg-emerald-600 hover:text-white hover:border-emerald-600 transition-all flex items-center justify-center gap-2"
                    >

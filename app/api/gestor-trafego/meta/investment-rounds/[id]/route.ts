/**
 * PUT    /api/gestor-trafego/meta/investment-rounds/[id]   Atualiza janela/meta/label.
 * DELETE /api/gestor-trafego/meta/investment-rounds/[id]   Remove a rodada.
 * Escopo: a rodada precisa pertencer a uma banca atribuída ao gestor.
 */
...
async function loadRoundForGestor(req: NextRequest, id: string) {
  ...
  const { data: round, error } = await supabaseServiceRole
    ...
    .select('id, banca_id, consultor_id, data_inicial, data_final')
    .eq('id', id)
    .maybeSingle();
  if (error) return { error: errorResponse(error.message, 500) };
  if (!round) return { error: errorResponse('Rodada não encontrada.', 404) };
  if (!(await gestorTrafegoUserCanAccessBanca(userId, profile, String(round.banca_id)))) {
    return { error: errorResponse('Você não tem permissão para esta rodada.', 403) };
  }
  return { round };
}
...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const loaded = await loadRoundForGestor(req, id);
    if (loaded.error) return loaded.error;
    const current = loaded.round!;

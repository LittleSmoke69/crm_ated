/**
 * POST /api/admin/zaplink/submissions/[id]/reassign
 * Move lead já atribuído para outra banca/gerente: atualiza submission, consultor (enroller + user_bancas),
 * notificação do gerente e envia WhatsApp (Loto Assistência).
 */
...
export async function POST(
  ...
  try {
    ...
    const { data: submission, error: subError } = await supabaseServiceRole
      ...
      .select('id, full_name, email, phone, status, consultor_user_id')
      .eq('id', submissionId)
      .single();
    ...
    // Consultor: novo enroller e nova banca
    await supabaseServiceRole
      .from('profiles')
    ...
    // Notificação: aponta para o novo gerente (uma notificação por submission)
    await supabaseServiceRole
      .from('zaplink_gerente_notifications')
      .update({ gerente_id: gerenteId })
      .eq('zaplink_submission_id', submissionId);

    // WhatsApp (Loto Assistência): avisa o lead da nova banca e gerente. Falha no envio não afeta a resposta.
    try {
      ...
      console.error('[zaplink/reassign] Erro ao enviar WhatsApp (Loto Assistência):', sendErr);
    }

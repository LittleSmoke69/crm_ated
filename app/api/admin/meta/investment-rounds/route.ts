/**
 * Rodadas de investimento de ADS por consultor.
 *
 * GET  /api/admin/meta/investment-rounds?banca_id=&consultor_id=
 *      Lista rodadas (filtra por banca e/ou consultor). Mais recentes primeiro.
 * POST /api/admin/meta/investment-rounds
 *      Cria rodada. Body: { banca_id, consultor_id, data_inicial, data_final, meta_gasto, label? }
 *      consultor_email é resolvido a partir do profile. Valida sobreposição de janelas
 *      no mesmo consultor.
 */
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Resolve email do consultor (snapshot).
    ...
    if (!profile?.email) {
      return errorResponse('Consultor não encontrado ou sem email.', 400);
    }

    // Valida sobreposição de janela no mesmo consultor.

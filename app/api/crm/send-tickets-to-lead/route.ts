...
/**
 * POST /api/crm/send-tickets-to-lead
 * Envia bilhetes de rifa para um lead (proxy para API Laravel sendTickets).
...
export async function POST(req: NextRequest) {
  try {
    ...
    const externalUrl = `${cleanBancaUrl}/api/crm/send-tickets`;
    ...
    if (!response.ok) {
      ...
      console.error('[CRM send-tickets-to-lead] HTTP', response.status, text);
      return errorResponse(`Erro ao enviar bilhetes: ${response.status}`);
    }
    ...
    console.error('[CRM send-tickets-to-lead]', msg);
    return errorResponse(msg || 'Erro ao enviar bilhetes.');
  }
}

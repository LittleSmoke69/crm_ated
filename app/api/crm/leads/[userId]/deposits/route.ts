    ...
    if (!response.ok) {
      if (response.status === 404) {
        return successResponse({
          success: true,
          message: 'Deposit history by user',
          total_value: 0,
          history: [],
          pagination: {
            current_page: 1,
      ...
      console.error(`[CRM Deposits] Erro HTTP ${response.status}:`, errorText);
      return errorResponse(`Erro ao buscar histórico de depósitos: ${response.status}`);
    }

    const result = await response.json();
    return successResponse(result);
  } catch (error: any) {
    console.error('[CRM Deposits] Erro:', error);
    return errorResponse(error.message || 'Erro ao buscar histórico de depósitos');
  }
}

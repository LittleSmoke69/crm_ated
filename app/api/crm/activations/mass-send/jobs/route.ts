...
export async function GET(req: NextRequest) {
  try {
    ...
    const { data: jobs, error } = await supabaseServiceRole
      ...
      .select(
        'id, message_id, message_title, instance_name, instance_names, group_ids, status, total_groups, sent_count, failed_count, processed_index, last_error, inter_group_delay_ms, created_at, updated_at'
      )
      .eq('user_id', userId)

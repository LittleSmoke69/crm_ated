/**
 * GET /api/anti-spam/events?config_id=...&limit=50
 * Lista eventos de entrada em grupos (quem entrou).
...
export async function GET(req: NextRequest) {
  try {
    ...
    const { data: config, error: cfgErr } = await supabaseServiceRole
      ...
      .select('id, master_instance_id, watcher_instance_id, owner_type, owner_id')
      .eq('id', configId)
      .single();

    if (cfgErr || !config) return errorResponse('Configuração não encontrada', 404);

    if (config.owner_type === 'user' && config.owner_id !== userId) {
    ...
    if (instanceNames.length === 0) return successResponse([], { pagination: { total: 0 } });
    ...
    for (const r of rows || []) {
      ...
      if (getAction(payload) !== 'add') continue;
    ...
    return successResponse(paged, { pagination: { total, page, limit } });
  } catch (err: any) {

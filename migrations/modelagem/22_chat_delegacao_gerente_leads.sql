-- Sincroniza a delegação admin -> gerente entre Chat e CRM.
...
BEGIN
  ...
  SELECT count(*) INTO v_count FROM chat_conversations c
  WHERE c.id = ANY(p_conversation_ids)
    AND (v_actor.status = 'super_admin' OR c.workspace_id = v_actor.zaploto_id)
    AND (v_actor.status IN ('super_admin', 'admin') OR c.gerente_id IS NULL OR c.gerente_id = v_actor.id);
...
-- Repara delegações já realizadas antes desta correção, sem tomar leads que já
-- tenham sido repassados a um captador.
UPDATE crm_leads l
   SET gerente_id = c.gerente_id,
       capture_status = 'pendente',
       assigned_by = c.assigned_by,
       assigned_at = COALESCE(c.assigned_at, l.assigned_at),
       updated_at = now()
  FROM chat_conversations c
 WHERE c.lead_id = l.id
   AND c.gerente_id IS NOT NULL
   AND c.user_id IS NULL
   AND c.assignment_status = 'atribuido'
   AND l.user_id IS NULL
   AND l.gerente_id IS DISTINCT FROM c.gerente_id;

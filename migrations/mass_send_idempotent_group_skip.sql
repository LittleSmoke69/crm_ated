-- Disparo em massa: suporte a “pular” grupo já enviado com sucesso sem duplicar contagem.
--
-- 1) RPC increment_mass_send_job_counts com p_expected_processed_index (CAS) e retorno BOOLEAN.
-- 2) Quando p_group_outcomes IS NULL (ou array vazio implícito no IF), não insere em
--    activation_mass_send_job_groups — apenas avança processed_index se p_sent/p_failed = 0.
-- 3) Índice parcial para listar group_id com success = true por job (worker em memória).

-- Remove assinatura antiga (sem p_expected_processed_index)
...
COMMENT ON FUNCTION public.increment_mass_send_job_counts(UUID, INT, INT, INT, INT, TEXT, TEXT, TIMESTAMPTZ, JSONB) IS
  'Atualiza activation_mass_send_jobs com CAS em processed_index (= p_expected_processed_index). '
  'Retorna FALSE se outro worker já avançou. p_group_outcomes NULL + p_sent/p_failed 0: só avança índice (skip idempotente para grupo já com sucesso em job_groups).';
...
COMMENT ON INDEX public.idx_amsg_job_groups_job_id_success_true IS
  'Acelera SELECT de group_id por job onde o disparo já teve sucesso (idempotência no worker).';

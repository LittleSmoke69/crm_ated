-- Lucro/aposta “realizado” deve refletir a transferência ao consultor (standard ou gerente→consultor),
-- não a reserva intermediária admin→estoque (admin_to_gerente_stock), para não duplicar com o repasse
-- do gerente que herda os mesmos snapshots.
-- Quem executa a transferência (admin vs gerente) não altera a fórmula — só o tipo de log.
...
AS $$
  WITH
    ...
    entries_vinculadas_com_deposito AS (
      ...
      INNER JOIN admin_lead_transfer_logs lg ON lg.id = e.transfer_log_id AND lg.banca_id = e.banca_id
      ...
      WHERE e.resolution_status = 'vinculado'
        ...
        AND COALESCE(lg.transfer_kind, 'standard') <> 'admin_to_gerente_stock'
    ),
...
COMMENT ON FUNCTION get_resolved_transfer_stats(uuid[], date, date, text) IS
  'Totais de transferências resolvidas e lucro realizado. Lucro/aposta ignoram transfer_kind=admin_to_gerente_stock (estoque intermediário); repasse gerente→consultor conta como transferência ao consultor.';
...
COMMENT ON FUNCTION get_resolved_transfer_stats(uuid[], date, date, text, uuid) IS
  'Totais de transferências resolvidas e lucro; opcional p_performed_by_user_id (gerente). Lucro/aposta ignoram admin_to_gerente_stock; gerente_stock_to_consultant segue a mesma regra que standard.';

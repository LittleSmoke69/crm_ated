...
async function listGestorUserIdsInOwnerSubtree(ownerId: string): Promise<string[]> {
  const out: string[] = [];
  const seen = new Set<string>();
  let frontier = [String(ownerId).trim()].filter(Boolean);
  const CHUNK = 80;
...
export async function buildGestorUserIdsByCrmBancaIdMap(
  ...
  for (const row of userBancasRows ?? []) {
    ...
    for (const bid of bancaIdsRow) {
      ...
      const cur = idsFromUserBancasByBanca.get(bid) ?? [];
      if (!cur.includes(userId)) cur.push(userId);
      idsFromUserBancasByBanca.set(bid, cur);
    }
  }
  ...
  const subtreeGestorIdsByOwner = new Map<string, string[]>();
  const uniqueOwnerIds = Array.from(new Set(Array.from(ownerByBancaId.values())));
  for (const oid of uniqueOwnerIds) {
    subtreeGestorIdsByOwner.set(oid, await listGestorUserIdsInOwnerSubtree(oid));
  }

  for (const bid of ids) {
    const ownerId = ownerByBancaId.get(bid);
    const hierarchyIds = ownerId ? (subtreeGestorIdsByOwner.get(ownerId) ?? []) : [];
    const ubIds = idsFromUserBancasByBanca.get(bid) ?? [];
    const merged = mergeUniqueIds(hierarchyIds, ubIds);

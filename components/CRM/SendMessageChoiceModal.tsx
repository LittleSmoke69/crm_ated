...
const SendMessageChoiceModal: React.FC<SendMessageChoiceModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[100] p-4">
      <div className="bg-gray-100 border border-gray-200 rounded-2xl w-full max-w-md shadow-2xl overflow-hidden">
        {/* Header */}
        <div className="p-6 border-b border-gray-200 flex items-center justify-between">
          <div>
            <h2 className="text-gray-800 font-bold text-lg">Enviar mensagem</h2>
            <p className="text-sm text-gray-600 mt-1">{messageTitle}</p>
          </div>
          <button 
            onClick={onClose}
        ...
        <div className="p-6 space-y-4">
          ...
          <button
            onClick={onSchedule}
            className="w-full p-4 bg-white hover:bg-gray-50 border-2 border-[#8CD955] text-[#8CD955] font-bold rounded-xl transition-all flex items-center justify-center gap-3"
          >

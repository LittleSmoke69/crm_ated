...
export default function VerifyGroupsOverlay({ isActive }: VerifyGroupsOverlayProps) {
  ...
  return (
    <div
      className="rounded-xl border-2 border-[#E86A24]/60 bg-[#E86A24]/5 dark:bg-[#E86A24]/10 shadow-lg overflow-hidden mb-6 animate-in fade-in duration-300"
      role="status"
      aria-live="polite"
    ...
    >
      <div className="flex items-center gap-4 p-4 md:p-5">
        <div className="relative flex shrink-0">
          <div className="absolute inset-0 rounded-full bg-[#E86A24]/30 animate-ping" />
          <div className="relative flex h-12 w-12 items-center justify-center rounded-full bg-[#E86A24] text-white">
            <Loader2 className="h-6 w-6 animate-spin" />
          </div>
        </div>
        <div className="min-w-0 flex-1">
          <p className="text-sm font-semibold text-gray-900 dark:text-gray-100 flex items-center gap-2">
            <Icon className="h-4 w-4 text-[#E86A24] shrink-0" />
            {step.label}
          </p>
      ...
      <div className="h-1.5 w-full bg-gray-200 dark:bg-gray-700 overflow-hidden rounded-b-xl">
        <div className="h-full w-1/3 bg-[#E86A24] rounded-r-full animate-verify-progress" />
      </div>
    </div>
  );
}

...
const InstanceList: React.FC<InstanceListProps> = ({ instances, onViewAll }) => {
  ...
  const getStatusStyle = (status: string) => {
    if (status === 'connected') return { color: '#8CD955' };
    return {};
  };
        ...
        {displayInstances.length === 0 ? (
          ...
          displayInstances.map((instance, index) => (
            <div
              ...
              className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg hover:bg-[#8CD95515] hover:border-[#8CD95540] border border-transparent transition-all duration-200 cursor-pointer"
            >
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  {!!instance.is_blocked_for_instances && (
                    <span
                      className="px-1.5 py-0.5 rounded text-xs font-medium bg-orange-100 text-orange-800 flex items-center gap-1"
